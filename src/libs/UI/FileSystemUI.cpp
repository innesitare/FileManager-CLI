...
void FileSystemUI::setQuitFunction(std::function<void()> q)
{
    quit = std::move(q);
}
...
void FileSystemUI::updateMainView(size_t cursorPosition)
{
...
void FileSystemUI::updateParentView()
{
...
void FileSystemUI::updateOperationView()
{
...
void FileSystemUI::updateSelectedCounter()
{
    statusSelected = L"(" + std::to_wstring(filesystemOperations.countSelectedFiles()) + L") ";
}

void FileSystemUI::updateAllUi(size_t cursorPosition)
{
...
void FileSystemUI::changePathAndUpdateViews(const std::filesystem::path& newPath)
{
...
void FileSystemUI::selectFile(const std::filesystem::path& selectedFile)
{
    filesystemOperations.appendSelectedFiles(selectedFile);
    updateSelectedCounter();
}

void FileSystemUI::toggleHiddenFiles()
{
...
void FileSystemUI::selectOperation(FileSystemOperations::Operation o)
{
    filesystemOperations.setOperation(o);
    updateOperationView();
}

void FileSystemUI::performOperation(const std::filesystem::path& dest)
{
    try {
...
// clang-format off
ftxui::Element FileSystemUI::Render()
{
    ...
    return
        window(
            text(L"Dan's File Manager") | bold | center,
            vbox({
                currentPathView,
                separator(),
                mainView | flex,
...
bool FileSystemUI::OnEvent(ftxui::Event event)
{
